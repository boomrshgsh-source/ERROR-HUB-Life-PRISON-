        ...
        if not char then
            ...
            if onScreen then
                ...
                if healthPercent > 0.6 then
                    d.HealthBar.Color = Color3.fromRGB(0, 255, 0) -- เขียว
                elseif healthPercent > 0.3 then
                    d.HealthBar.Color = Color3.fromRGB(255, 255, 0) -- เหลือง
                else
                    d.HealthBar.Color = Color3.fromRGB(255, 0, 0) -- แดง
                end
...
local function startESP()
    ...
    Players.PlayerAdded:Connect(function(p)
        if espEnabled and p ~= LocalPlayer then createDrawsFor(p) end
    end)
